...
class GraphicTracker<T> extends Tracker<T> {
    ...
    private ITrackerProcess<T> iTrackerProcess;

    GraphicTracker(GraphicOverlay overlay, TrackedGraphic<T> graphic, ITrackerProcess<T> iTrackerProcess) {
        mOverlay = overlay;
        mGraphic = graphic;
    ...
    public void onUpdate(Detector.Detections<T> detectionResults, T item) {
        iTrackerProcess.ValueReaded(detectionResults);
//descomentar esto para ver el tracking
